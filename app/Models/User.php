...
class User extends Model
{
    ...
    public static function getOrCreate(
        ...
        if (!is_null($user)) {
            if ($user->bot_id != $bot->id) {
                //Обновление последнего бота
                $bots_ids = json_decode($user->bots_ids, true);
                if (!in_array($bot->id, $bots_ids)) {
                    //Пользователь уже был, но в боте новый
                    $bots_ids[] = $bot->id;
            ...
            if ($user->tg_username != $tg_username) {
                //Если обновилось имя пользователь в ТГ
    ...
    // TODO: связь historyBalance отключена, потому что для HistoryBalance
    // нет актуальной таблицы/миграции в текущей схеме БД.

    private static function getRefUser($tg_id, $bot)
    {
        if (!empty($tg_id)) {
            //Если в /start передан реферал
            $refUser = self::where("tg_id", $tg_id)->first();
        }

        if (
            (!isset($refUser) || is_null($refUser)) &&
            !empty($bot->user_id) &&
            !is_null($bot->user_id)
        ) {
            //Если есть владелец бота - он реферал
            $refUser = self::find($bot->user_id);
        }

        if (isset($refUser)) {
            // TODO: поле count_ref отсутствует в актуальных миграциях.
    ...
    public static function addAdminsWaitSendMessage(
        $msg,
        $buttons = [],
        $photo = "",
        $botId = null,
    ...
    public function addWaitSendMessage(
        $msg,
        $buttons = [],
        $photos = [],
        $send_from = 0,
        ...
        //Добавление в отложеные сообщения
