...
trait shared
{
    ...
    private function getReferralLink(): string
    {
        if (!empty($this->main->bot->bot_name)) {
            return 'https://t.me/' . $this->main->bot->bot_name . '?start=' . $this->main->user->tg_id;
        }

        return 'Ссылка станет доступна после настройки username бота';
    }

    private function getHelpContact(): string
    {
        return $this->main->bot->support_contact ?? '@support';
    }
    ...
    private function buildCampaignSummaryText(string $phone, string $name, string $mode): string
    {
        ...
        return implode("\n", [
            ...
            'Баланс после списания: <b>' . $this->formatMoney($balanceAfter) . '</b>',
        ]);
    }
    ...
    private function buildStartText(): string
    {
        return implode("\n", [
            ...
            'Баланс: <b>' . $this->formatMoney($this->main->user->balance) . '</b>',
    ...
    {
        $lastPaymentsCount = Pay::where('user_id', $this->main->user->id)->count();
        ...
        return implode("\n", [
            ...
            'Начислено по рефке: <b>' . $this->formatMoney($this->getReferralBonusAmount()) . '</b>',
